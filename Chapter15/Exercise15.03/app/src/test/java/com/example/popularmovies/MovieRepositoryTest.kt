...
class MovieRepositoryTest {
    ...
    @Test
    fun fetchRemoteMoviesFromNetwork() {
        val movies = listOf(Movie(id = 3), Movie(id = 4))
        val response = PopularMoviesResponse(1, movies)

        val dao = object : MovieDao {
            val daoMovies = mutableListOf<Movie>()

            override fun addMovies(movies: List<Movie>) {
                daoMovies.addAll(movies)
            }

            override fun getMovies(): List<Movie> = daoMovies
        ...
        runTest {
            ...
            assertEquals(movies, dao.getMovies())
        }
    }
}